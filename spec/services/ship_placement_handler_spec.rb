...
RSpec.describe ShipPlacementHandler do
  let(:input_helper) { instance_double(InputHelper, get_battleship_coordinates: { start_coordinate: [0, 0], end_coordinate: [0, 1] }) }
  let(:game_interface) { instance_double(GameInterface) }
  let(:ship) { instance_double(Ship, valid?: true, get_coordinates: [[0, 0], [0, 1]]) }
  let(:board) { instance_double(Board, board_printer: board_printer, add_ship: true, occupy_cell: true, cell_occupied?: false) }
  ...
  before do
    ...
    allow(game_interface).to receive_messages(ship_placement_message: true, ship_placement_error_message: true, wait_to_continue: true)
  end
