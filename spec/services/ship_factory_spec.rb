  ...
  describe '.build_ship' do
    context 'when length is 1' do
      it 'builds a drone boat' do
        expect(Ship).to receive(:create_drone_boat).with(start_coordinate: start_coordinate,
    ...
    context 'when length is 2' do
      it 'builds a patrol boat' do
        expect(Ship).to receive(:create_patrol_boat).with(start_coordinate: start_coordinate,
                                                          end_coordinate: end_coordinate)
    ...
    context 'when length is 3' do
      it 'builds a submarine' do
        expect(Ship).to receive(:create_submarine).with(start_coordinate: start_coordinate,
                                                        end_coordinate: end_coordinate)
    ...
    context 'when length is 4' do
      it 'builds a destroyer' do
        expect(Ship).to receive(:create_destroyer).with(start_coordinate: start_coordinate,
                                                        end_coordinate: end_coordinate)
    ...
    context 'when length is invalid' do
      it 'raises an ArgumentError' do
        expect do
          ShipFactory.build_ship(length: 5, start_coordinate: start_coordinate,
                                 end_coordinate: end_coordinate)
        end.to raise_error(ArgumentError, 'Invalid ship length: 5')
      end
    end
  end
end
